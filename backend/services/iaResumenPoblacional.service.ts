...
export interface HallazgosDepartamento {
  nombre: string;
  hallazgos: string[];
}
...
export interface ResumenIAResult {
  ...
  departamentos: HallazgosDepartamento[];
  modelo: string;
}

export async function generarResumenMedicoIA(payloadAgregados: unknown): Promise<ResumenIAResult> {
  ...
  let parseado: { resumenGeneral: ResumenIAResult["resumenGeneral"]; departamentos: HallazgosDepartamento[] };
  try {
