...
function ConfigController(db) {
    ...
    const ObtenerUsuarios = async (req, res) => {
        try {
            ...
            const result = await executeConnection(sql, params_web_service_1.TipoConsulta.Consulta, []);
            return res.json({
                ok: true,
                data: result
            });
    ...
    const ObtenerCuentaAD = async (req, res) => {
        try {
            ...
            const result = await executeConnection("[TNGCORE].[dbo].[WSP_Consumibles_ConsultarMatriculaAD]", params_web_service_1.TipoConsulta.ProcedimientoAlmacenado, params);
            return res.json({
                ok: true,
                data: result
            });
    ...
    const EdicionUsuarios = async (req, res) => {
        try {
    ...
    const VerificarMatricula = async (req, res) => {
        try {
            const { matricula } = req.body;
            const params = [{ Nombre: "@Matricula", Valor: String(matricula !== null && matricula !== void 0 ? matricula : "") }];
            const result = await executeConnection("SELECT ID FROM [TNGCORE].[dbo].[SCII_Usuarios] WHERE Matricula = @Matricula", params_web_service_1.TipoConsulta.Consulta, params);
