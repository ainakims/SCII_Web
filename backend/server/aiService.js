...
exports.analyzeConsult = async (consultData) => {
    ...
    if (SignosVitales.TA) {
        const [sysStr, diaStr] = SignosVitales.TA.split('/');
        const sys = parseInt(sysStr, 10);
        const dia = parseInt(diaStr, 10);

        if (sys >= 180 || dia >= 120) {
            riskScore += 10;
            analysis.inconsistencias.push(`Presión Arterial Crítica (${SignosVitales.TA} mmHg): Posible Crisis Hipertensiva.`);
            ...
            riskScore += 3;
            analysis.inconsistencias.push(`Presión Arterial Elevada (${SignosVitales.TA} mmHg) fuera de metas.`);
            ...
            riskScore += 5;
            analysis.inconsistencias.push(`Hipotensión (${SignosVitales.TA} mmHg).`);
    ...
    if (SignosVitales.IMC) {
        if (SignosVitales.IMC >= 30) {
            ...
            analysis.inconsistencias.push(`Sobrepeso (IMC: ${SignosVitales.IMC}).`);
        }
    }

    // Diagnóstico de Síndrome Metabólico Reuniendo Criterios
    if (metabolicSyndromeCount >= 2 && SignosVitales.TA && (parseInt(SignosVitales.TA.split('/')[0]) >= 130 || parseInt(SignosVitales.TA.split('/')[1]) >= 85)) {
    ...
    if (analysis.diagnosticoDiferencial.length === 0) {
        ...
        analysis.inconsistencias.push('Parámetros biológicos dentro de rangos normales o estables.');
    }
