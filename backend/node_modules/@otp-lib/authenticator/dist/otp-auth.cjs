...
var core = require("@otp-lib/core"), utils = require("@otp-lib/core/utils");
...
var Authenticator = class {
    ...
    constructor(otp, options) {
        if (!utils.TypeGuard.isPlainObject(options)) {
            throw new Error("The options must be a plain object");
        }
        const account = options.account;
        if (!utils.TypeGuard.isString(account)) {
            throw new Error("The options.account must be a string");
        }
        const issuer = options.issuer ?? null;
        if (issuer !== null && !utils.TypeGuard.isString(issuer)) {
    ...
    static fromURL(url) {
        ...
        } else if (!utils.TypeGuard.isEnum(url.host, AuthenticatorType)) {
        ...
        options.secret = core.Secret.fromBase32(secret);
        const algorithm = url.searchParams.get("algorithm");
        if (algorithm !== null) {
            const value = algorithm.toUpperCase();
            if (!utils.TypeGuard.isKeyOf(value, core.HashAlgorithms)) {
                throw new Error("The authenticator URI algorithm is not supported");
            }
            options.algorithm = core.HashAlgorithms[value];
        }
        const digits = url.searchParams.get("digits");
        if (digits !== null) {
            const value = parseInt(digits, 10);
            if (!utils.TypeGuard.isPositiveInteger(value)) {
...
var HOTPAuthenticator = class extends authenticator_default {
    constructor(options) {
        super(new core.HOTP(options), options);
    }
    setCounter(counter) {
...
var hotp_default = HOTPAuthenticator;

var TOTPAuthenticator = class extends authenticator_default {
    constructor(options) {
        super(new core.TOTP(options), options);
    }
    getWindow() {
...
var totp_default = TOTPAuthenticator;

Object.defineProperty(exports, "HashAlgorithms", {
    enumerable: true,
    get: function() {
        return core.HashAlgorithms;
    }
});
...
exports.Authenticator = authenticator_default;

exports.AuthenticatorType = AuthenticatorType;

exports.HOTP = hotp_default;

exports.TOTP = totp_default;//# sourceMappingURL=otp-auth.cjs.map