...
const KpiCard: React.FC<KpiCardProps> = ({ icon, label, value }) => (
  <div className="relative bg-white rounded-xl overflow-hidden flex items-center px-5 py-4 cursor-default group shadow-xs">
    <div className="relative z-1 size-12 rounded-md bg-gray-100/60 flex items-center justify-center flex-shrink-0 shadow-xs transition-transform duration-300">
      <i className={`${icon.includes(" ") ? icon : `fa-solid fa-${icon}`} text-xl bg-linear-to-b from-sea-blue to-sky-blue bg-clip-text text-transparent`}></i>
    </div>
    <div className="ml-4 flex-1 relative z-1 min-w-0">
      ...
      <h2 className="text-xl font-bold text-sea-blue leading-tight">{value}</h2>
